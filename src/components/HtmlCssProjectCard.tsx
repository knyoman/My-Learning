...
import { HtmlCssProject } from '../data/htmlCssProjects';
import { Check, Star, MessageCircle, ExternalLink, Edit, Save, X, Github } from 'lucide-react';
import { useLocalStorage } from '../hooks/useLocalStorage';
import { Confetti } from './Confetti';

interface HtmlCssProjectCardProps {
  project: HtmlCssProject;
  onEdit: (project: HtmlCssProject) => void;
  onToggleComplete: (projectId: number) => void;
}

export const HtmlCssProjectCard: React.FC<HtmlCssProjectCardProps> = ({ project, onEdit, onToggleComplete }) => {
  const [completedTasks, setCompletedTasks] = useLocalStorage<number[]>('htmlCssCompletedTasks', []);
  const [projectNotes, setProjectNotes] = useLocalStorage<{ [key: number]: string }>('htmlCssProjectNotes', {});
  const [projectLinks, setProjectLinks] = useLocalStorage<{ [key: number]: string }>('htmlCssProjectLinks', {});
  const [githubLinks, setGithubLinks] = useLocalStorage<{ [key: number]: string }>('htmlCssGithubLinks', {});
  const [showNotes, setShowNotes] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editedProject, setEditedProject] = useState(project);
  const [editedLink, setEditedLink] = useState(projectLinks[project.id] || '');
  const [editedGithubLink, setEditedGithubLink] = useState(githubLinks[project.id] || '');

  const isCompleted = completedTasks.includes(project.id);
  const currentNote = projectNotes[project.id] || '';
  const currentLink = projectLinks[project.id] || '';
  const currentGithubLink = githubLinks[project.id] || '';
  ...
  const handleSaveEdit = () => {
    ...
    setProjectLinks(prev => ({
      ...prev,
      [project.id]: editedLink
    }));
    setGithubLinks(prev => ({
      ...prev,
      [project.id]: editedGithubLink
    }));
  ...
  const handleCancelEdit = () => {
    ...
    setEditedLink(projectLinks[project.id] || '');
    setEditedGithubLink(githubLinks[project.id] || '');
    setIsEditing(false);
  };
  ...
  return (
    ...
    `}>
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-orange-600/5 via-red-600/5 to-pink-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className="relative p-6 z-10">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                {!isEditing ? (
                  ...
                  <select
                    ...
                    className="px-2 py-1 rounded-full text-xs bg-gray-800 text-white border border-orange-500/30"
                  >
              ...
              {!isEditing ? (
                ...
                <input
                  ...
                  className="w-full bg-gray-800/50 border border-orange-500/30 rounded-lg px-3 py-1 text-white text-sm"
                />
              )}
            </div>
          </div>
            ...
            <button
              ...
              className={`
                ...
                ${isCompleted 
                  ...
                  : 'bg-orange-500/20 hover:bg-orange-500 text-orange-300 hover:text-white'
                }
              `}
            >
              <Check size={16} />
        ...
        {!isEditing ? (
          ...
          <textarea
            ...
            className="w-full bg-gray-800/50 border border-orange-500/30 rounded-lg p-3 text-white text-sm resize-none h-20 mb-4"
          />
        )}
        ...
        {isEditing && (
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full bg-gray-800/50 border border-orange-500/30 rounded-lg p-3 text-white placeholder-gray-400 text-sm focus:outline-none focus:border-orange-400 transition-colors"
            />
          </div>
        )}
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="p-4 rounded-lg bg-white/5 border border-orange-500/30">
              <textarea
                ...
                className="w-full h-20 bg-transparent border border-orange-500/30 rounded-lg p-3 text-white placeholder-gray-400 text-sm resize-none focus:outline-none focus:border-orange-400 transition-colors"
              />
            </div>
          </div>
        )}
